...
describe('typed provisional MEAL API', () => {
  it('fails closed before dispatch in production', async () => {
    const request = vi.fn<MealRequestDispatcher>();
  ...
  it('uses the MEAL management list, never the general user poll list', async () => {
    const request = vi.fn<MealRequestDispatcher>().mockResolvedValue({
  ...
  it('uses backend-filtered own-account and own-settlement endpoints', async () => {
    const request = vi.fn<MealRequestDispatcher>()
      ...
      .mockResolvedValueOnce({accounts: [], summary: {actualTotalAmount: 0, chargedMemberCount: 0, requestedTotalAmount: 0, roundingAdjustment: 0}});
    const api = createMealApi({isMockMode: () => true, request});
  ...
  it('dispatches POST/DELETE duty operations without changing a campus role', async () => {
    const request = vi.fn<MealRequestDispatcher>()
      .mockResolvedValueOnce({assignmentId: 20, campusId: 1, userId: 9, name: '밥담당', email: 'meal@example.test', dutyType: 'MEAL', isActive: true, assignedAt: '2026-07-13T03:00:00.000Z'})
  ...
  it('sends one batch charge request and never the superseded option endpoint', async () => {
    ...
    expect(request.mock.calls[0]?.[0]).not.toContain('meal-charge-groups');
  });
});
