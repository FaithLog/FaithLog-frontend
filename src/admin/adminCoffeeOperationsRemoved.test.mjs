...
describe('admin coffee operations separation', () => {
  it('keeps coffee duty assignment but removes coffee poll creation and settlement controls', () => {
    expect(source).toContain("{id: 'coffee', label: '커피담당'}");
    expect(source).not.toContain("{id: 'COFFEE', label: '커피 주문'}");
    expect(source).not.toContain("{id: 'COFFEE', label: '커피'}");
    expect(source).not.toContain('accessibilityLabel="커피 미납자 푸시 알림 발송 확인 열기"');
  });

  it('filters coffee polls and templates out of the admin management state', () => {
    expect(source).toContain("polls.filter((poll) => poll.pollType !== 'COFFEE')");
